...
const Test = () => {
  ...
  const updateScore = () => {
    ...
    if ((score / Total) * 100 >= 60) {
      ...
      axios
        .post(
          `https://localhost:44388/api/test/${courseID}/${levelID}/update`,
          postData
        )
        .then((res) => {
  ...
  return (
    <div>
      ...
      {showSheet ? (
        <div
          style={{ height: "100vh", color: "white", background: "#261A40" }}
          className={styles.starterBg}
        >
          <div style={sheetStyle}>
            {newData.map((item) => (
              <div style={sheetQuestionStyle}>
                {
                  <div>
                    Q.{item.QuesID} {item.Question}{" "}
                    {item.Options.map((option) => (
                      <div style={sheetOptionStyle}>
                        <p
                          style={
                            option.IsCorrect
        ...
        >
          ...
          >
            ...
            >
              <div>
                {levelData.map((item) => (
                  <div>
                    <div
                      style={item.HasTaken ? testtakenstyle : testnottakenstyle}
                      onClick={
                        item.NextInLine ? () => handleTest(item.LevelID) : {}
                      }
                    >
                      {item.LevelName}
                      {item.NextInLine ? (
                        <p style={{ marginLeft: "1rem" }} role="button">
                          <img
            ...
            <div
              style={cardStyle}
              className="flex justify-evenly items-center w-3/5 h-72"
            >
              {showScore ? (
                ...
                <>
                  <div className="question-section">
                    <div className="mb-5">
                      {Total !== 0 ? (
                        <p>
                          <span>Question {currentQuestion + 1}</span>/{Total}
                        </p>
                      ) : (
                  ...
                  <div className="flex  flex-col ">
                    {OptData[currentQuestion]?.map((item) => (
                      <button
                        ...
                        className="mb-5"
                        onClick={() =>
                          handleAnswerOptionClick(item.IsCorrect, item.Answer)
                        }
                      >
                        {item.Answer}
                      </button>
